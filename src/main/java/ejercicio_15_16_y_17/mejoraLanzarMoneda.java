 ...
 * @author aleja
 */
public class mejoraLanzarMoneda {
    
    //atributo del lanzamonedas
    private boolean[] monedas = new boolean[1000];//longitud del array

    public mejoraLanzarMoneda() {
    }//como no hay nada, todo es nulo, entonces el array se inicualiza a false
    
    public mejoraLanzarMoneda(int cantidadDeTiradas){
        //como es un constructor con parametors,actualizamos la longitud del 
        //array hasta la nueva cantidad
        this.monedas = new boolean[cantidadDeTiradas];
    }
    ...
    public boolean[] llenarArray() {
        ...
        for (int i = 0; i < monedas.length; i++) {
            ...
            this.monedas[i] = mejoraLanzarMoneda.lanzar();
